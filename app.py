...
with mysql.connector.connect(host=host,port=port,user=user,password=password,db=db) as conn:
    ...
    cursor.execute("CREATE TABLE if not exists user (username varchar(255) NOT NULL,mobile_no bigint NOT NULL,email varchar(155) NOT NULL,address text NOT NULL,password varbinary(255) DEFAULT NULL,PRIMARY KEY (email),UNIQUE KEY mobile_no (mobile_no))")
    cursor.execute("CREATE TABLE if not exists additems (itemid binary(16) NOT NULL,item_name longtext NOT NULL,dis longtext NOT NULL,qyt int DEFAULT NULL,category enum(electronics,home,fashion,grocery) DEFAULT NULL,price int DEFAULT NULL,added_by varchar(255) DEFAULT NULL,img_id varchar(255) DEFAULT NULL,PRIMARY KEY (itemid),KEY added_by (added_by),CONSTRAINT additems_ibfk_1 FOREIGN KEY (added_by) REFERENCES vendor (email) ON DELETE CASCADE ON UPDATE CASCADE)")
    cursor.execute("CREATE TABLE if not exists orders (ordid binary(16) NOT NULL,itemid binary(16) NOT NULL,item_name varchar(255) DEFAULT NULL,qty int DEFAULT NULL,total_price decimal(20,4) DEFAULT NULL,user varchar(155) DEFAULT NULL,category varchar(255) DEFAULT NULL,imgid varchar(255) DEFAULT NULL,dis text,PRIMARY KEY (ordid),KEY itemid (itemid),KEY user (user),CONSTRAINT orders_ibfk_1 FOREIGN KEY (itemid) REFERENCES additems (itemid) ON DELETE CASCADE,CONSTRAINT orders_ibfk_2 FOREIGN KEY (user) REFERENCES user (email) ON DELETE SET NULL ON UPDATE CASCADE)")
